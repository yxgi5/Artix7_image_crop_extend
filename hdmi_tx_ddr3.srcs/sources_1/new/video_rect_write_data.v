...
(
	...
	output reg                  write_req,           // Start reading a frame of data     
	input                       write_req_ack,       // Read request response
	output reg[1:0]             write_addr_index,
	output reg[1:0]             read_addr_index,
	output                      write_en,            // Read data enable
//	input[DATA_WIDTH - 1:0]     write_data,          // Read data
	input                       timing_hs,    
	input                       timing_vs,    
	input                       timing_de,    
...
begin
	...
	else if(pos_vs_d0 == 1'b1 && pos_vs == 1'b0)
...
//	if(rst == 1'b1)
//		write_req <= 1'b0;
//	else if(timing_vs_d0 == 1'b1 && timing_vs_d1 == 1'b0)
//		write_req <= 1'b1;
//	else if(write_req_ack == 1'b1)
//		write_req <= 1'b0;
//end
...
//	else if(write_en_r_d0 == 1'b1)
//		vout_data_r <= write_data;
	else
		vout_data_r <= pos_data_d1;
end

timing_gen_xy#(.DATA_WIDTH(DATA_WIDTH)) timing_gen_xy_m1(
	.rst_n    (~rst          ),
	.clk      (video_clk     ),
	.i_hs     (timing_hs     ),
	.i_vs     (timing_vs     ),
